...
$home_menu_items = tb4cf_get_home_menu_source_items( $home_feed_limit );
$home_cards      = tb4cf_get_home_menu_highlight_cards( $home_menu_items, 6 );
  ...
  <div class="tb4c-container tb4c-home-menu-feed-container">
    <header class="tb4c-home-menu-feed-hero">
      ...
      <div class="tb4c-home-menu-feed-hero-actions">
        <a class="tb4c-btn tb4c-btn-primary" href="<?php echo esc_url( home_url( '/community/' ) ); ?>"><?php echo tb4cf_icon_markup( 'ph-users-three', '👥' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?><span>เปิดฟีดชุมชน</span></a>
      </div>
    </header>
      ...
      <nav class="tb4c-home-menu-chip-row" aria-label="เมนูหลักที่นำมาแสดงบนหน้าแรก">
        <?php foreach ( $home_menu_items as $menu_item ) : ?>
          <a href="<?php echo esc_url( $menu_item['url'] ); ?>">
            <?php echo tb4cf_icon_markup( $menu_item['icon'], $menu_item['fallback'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
    ...
    <div class="tb4c-home-menu-feed-layout">
            ...
            <?php foreach ( $home_cards as $index => $card ) : $preview = $card['preview']; ?>
              <article class="tb4c-home-menu-card <?php echo 0 === $index ? 'is-featured' : ''; ?>">
                <a class="tb4c-home-menu-card-media" href="<?php echo esc_url( $preview['url'] ); ?>" aria-label="<?php echo esc_attr( $preview['title'] ); ?>">
                  ...
                  <?php else : ?>
                    <span class="tb4c-home-menu-card-icon"><?php echo tb4cf_icon_markup( $card['icon'], $card['fallback'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
                  <?php endif; ?>
                </a>
                <div class="tb4c-home-menu-card-body">
                  <div class="tb4c-home-menu-card-topline">
                    <span><?php echo tb4cf_icon_markup( $card['icon'], $card['fallback'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?><?php echo esc_html( $card['label'] ); ?></span>
      ...
      <aside class="tb4c-home-menu-feed-side" aria-label="อัปเดตล่าสุด">
          ...
          <?php if ( $home_feed_posts->have_posts() ) : ?>
            <div class="tb4c-home-menu-mini-list">
              <?php while ( $home_feed_posts->have_posts() ) : $home_feed_posts->the_post(); ?>
                <a href="<?php echo esc_url( tb4cf_get_post_link( get_the_ID() ) ); ?>">
        ...
        <section class="tb4c-home-menu-side-card tb4c-home-menu-qc-card">
          ...
          <p>จัดวางให้ดูเป็นฟีดระดับโลกแบบเรียบ ไม่บังส่วนหัว/ส่วนท้าย และเชื่อมต่อเฉพาะหน้าชุมชนหลักอย่างชัดเจน</p>
        </section>
      </aside>
    </div>
  </div>
