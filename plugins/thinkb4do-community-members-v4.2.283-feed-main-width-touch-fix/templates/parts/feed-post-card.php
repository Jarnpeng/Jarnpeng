...
", $post_poll_options ) : '';
              ...
              $post_caption_text = tb4cm_media_visible_text( $post_caption_source, $post_media );
              $post_caption_text = wp_trim_words( wp_strip_all_tags( $post_caption_text ), 34 );
              $tb4c_title_for_compare   = trim( wp_strip_all_tags( get_the_title( $post_id ) ) );
              $tb4c_caption_for_compare = trim( wp_strip_all_tags( $post_caption_text ) );
              if ( $tb4c_caption_for_compare && $tb4c_title_for_compare && $tb4c_caption_for_compare === $tb4c_title_for_compare ) {
                  $post_caption_text = '';
              }
              if ( $tb4c_track_reels && ! empty( $post_media_album ) && ! in_array( (int) $post_id, $reel_item_ids, true ) ) {
                  $reel_items[] = [
                      ...
                      'author_url'     => tb4cm_social_profile_url( $author_id ),
                      ...
                      'topic_label'    => tb4cm_topic_label( $type ),
                      'visibility'     => function_exists( 'tb4cm_visibility_label' ) ? tb4cm_visibility_label( $visibility ) : ( 'members' === $visibility ? 'เฉพาะสมาชิก' : ( 'groups' === $visibility ? 'เฉพาะกลุ่ม' : 'สาธารณะ' ) ),
                      ...
                      'cover'          => has_post_thumbnail( $post_id ) ? get_the_post_thumbnail_url( $post_id, 'medium_large' ) : ( 'image' === ( $post_media['type'] ?? '' ) ? ( $post_media['url'] ?? '' ) : ( function_exists( 'tb4cm_get_video_poster_url' ) ? tb4cm_get_video_poster_url( $post_media ) : '' ) ),
          ...
          <article class="tb4c-post-card tb4c-social-post-card tb4c-ig-post tb4c-native-post <?php echo $has_visual_media ? 'tb4c-has-media' : 'tb4c-text-only-post'; ?>" data-post-id="<?php echo esc_attr( $post_id ); ?>">
                  ...
                  <?php if ( current_user_can( 'edit_post', $post_id ) || current_user_can( 'delete_post', $post_id ) ) : ?>
                    <div class="tb4c-feed-owner-tools tb4c-feed-owner-tools-inline" aria-label="จัดการโพสต์ของฉัน">
                      <?php if ( current_user_can( 'edit_post', $post_id ) ) : ?>
                        <button type="button" aria-label="แก้ไขโพสต์" title="แก้ไขโพสต์" data-tb4c-edit-post="<?php echo esc_attr( $post_id ); ?>" data-post-title="<?php echo esc_attr( get_the_title() ); ?>" data-post-content="<?php echo esc_attr( wp_strip_all_tags( get_the_content() ) ); ?>" data-post-type="<?php echo esc_attr( $type ); ?>" data-post-visibility="<?php echo esc_attr( $visibility ); ?>" data-post-media-url="<?php echo esc_attr( $post_media_url ); ?>" data-post-media-album="<?php echo esc_attr( $post_media_album_urls ); ?>" data-post-tags="<?php echo esc_attr( $tag_input_value ); ?>" data-post-feeling="<?php echo esc_attr( $post_feeling ); ?>" data-post-poll-question="<?php echo esc_attr( $post_poll_question ); ?>" data-post-poll-options="<?php echo esc_attr( $post_poll_options_value ); ?>"><?php echo tb4cm_icon_markup( 'ph-pencil-simple-line', '✎' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?><span>แก้ไข</span></button>
                      <?php endif; ?>
            ...
            <a class="tb4c-post-inline-text" href="<?php the_permalink(); ?>">
              <h2><?php the_title(); ?></h2>
            ...
            <div class="tb4c-post-actions tb4c-social-actions tb4c-ig-actions tb4c-ig-action-row tb4c-post-actions-balanced">
              ...
              <a href="<?php the_permalink(); ?>#comments" aria-label="ความคิดเห็น"><i class="ph ph-chat-circle"></i><span><?php echo esc_html( get_comments_number() ); ?></span></a>
              <button type="button" data-tb4c-copy-link="<?php echo esc_url( get_permalink() ); ?>" aria-label="แชร์"><i class="ph ph-paper-plane-tilt"></i><span>แชร์</span></button>
