...
$tb4c_feed_paged    = 1;
$query_args         = function_exists( 'tb4cm_get_community_feed_query_args' )
    ? tb4cm_get_community_feed_query_args( $active_topic, $search_query, $tb4c_feed_paged, $tb4c_feed_per_page )
    : [
...
if ( $rail_reel_query->have_posts() ) {
    while ( $rail_reel_query->have_posts() ) {
        ...
        $rail_album        = tb4cm_get_post_media_album( $rail_post_id, $rail_raw_text );
        ...
        if ( ! $rail_cover_url && ! empty( $rail_album[0] ) && is_array( $rail_album[0] ) ) {
            ...
            } elseif ( function_exists( 'tb4cm_get_video_poster_url' ) ) {
                $rail_cover_url = tb4cm_get_video_poster_url( $rail_album[0] );
            }
        }
        ...
        $rail_caption_text   = tb4cm_media_visible_text( $rail_caption_source, ! empty( $rail_album[0] ) ? $rail_album[0] : null );
        ...
        $rail_reel_items[] = [
            ...
            'author_url'    => tb4cm_social_profile_url( $rail_author_id ),
            ...
            'topic_label'   => tb4cm_topic_label( $rail_type ),
            'visibility'    => function_exists( 'tb4cm_visibility_label' ) ? tb4cm_visibility_label( $rail_visibility ) : ( 'members' === $rail_visibility ? 'เฉพาะสมาชิก' : ( 'groups' === $rail_visibility ? 'เฉพาะกลุ่ม' : 'สาธารณะ' ) ),
...
#tb4c-community-app.tb4c-v42231-main-no-sidebar .tb4c-module-grid{width:min(100%,760px);margin:0 auto;display:grid;grid-template-columns:minmax(0,720px);gap:0;align-items:start;justify-content:center;}
#tb4c-community-app.tb4c-v42231-main-no-sidebar .tb4c-social-feed{min-width:0;max-width:720px;width:100%;display:grid;gap:10px;}
...
@media(max-width:860px){#tb4c-community-app.tb4c-v42231-main-no-sidebar{width:100%;max-width:100%;margin:0;padding:7px 9px 30px;}#tb4c-community-app.tb4c-v42231-main-no-sidebar .tb4c-module-grid{display:block;width:100%;max-width:720px;}}
</style>
<section class="tb4c-shell tb4c-community-modular tb4c-social-shell tb4c-app-shell tb4c-native-shell tb4c-native-feed-page tb4c-v42231-main-no-sidebar" id="tb4c-community-app" data-tb4c-layout="main-no-sidebar-v42231" data-ui-version="4.2.234" data-tb4c-size-lock="true">
  ...
  <div class="tb4c-container tb4c-social-layout tb4c-app-layout tb4c-module-grid" data-tb4c-layout-controller="center-only">
    <?php
      ...
      $tb4c_center_module = tb4cm_get_module_file( 'center-feed' );
                ...
                <div class="tb4c-reel-phone-media">
                  ...
                  <?php elseif ( 'video' === $reel_media_type ) : ?>
                    <?php if ( preg_match( '~\.(mp4|m4v|webm|ogv|ogg)(\?.*)?$~i', $reel_media_url ) ) : ?>
                      <video class="tb4c-reel-video" playsinline preload="metadata" loop muted poster="<?php echo esc_url( tb4cm_get_video_poster_url( $reel_media ) ); ?>">
                  ...
                  <div class="tb4c-reel-content">
                    <div class="tb4c-reel-author-row">
                      <a class="tb4c-reel-author" href="<?php echo esc_url( $reel['author_url'] ); ?>">
                        <span class="tb4c-reel-author-avatar">
                          <?php if ( ! empty( $reel['author_avatar'] ) ) : ?><img src="<?php echo esc_url( $reel['author_avatar'] ); ?>" alt=""><?php else : ?><?php echo esc_html( tb4cm_get_initial( $reel['author_name'] ) ); ?><?php endif; ?>
                        </span>
                  ...
                  <div class="tb4c-reel-side">
                    ...
                    <a href="<?php echo esc_url( $reel['permalink'] ); ?>#comments" aria-label="ความคิดเห็น">
