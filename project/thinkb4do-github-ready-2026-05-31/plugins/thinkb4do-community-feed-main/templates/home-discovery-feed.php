    ...
    function tb4_home_v247_fallback_menu_feed() {
        return [
            [
                ...
                'description' => __( 'พื้นที่ติดตามเรื่องใหม่ พูดคุย และดูความเคลื่อนไหวที่เกี่ยวข้องกับชุมชน', 'thinkb4do' ),
                'image'       => '',
...
if ( ! function_exists( 'tb4_home_v248_get_media_cover' ) ) {
    function tb4_home_v248_get_media_cover( $post_id ) {
        ...
        if ( empty( $album ) && function_exists( 'tb4cf_get_post_media_album' ) ) {
            $album = tb4cf_get_post_media_album( $post_id, wp_strip_all_tags( get_post_field( 'post_content', $post_id ) ) );
        }
        ...
        if ( ! empty( $album[0] ) && is_array( $album[0] ) ) {
            ...
            if ( 'video' === $type && function_exists( 'tb4cf_get_video_poster_url' ) ) {
                $poster = tb4cf_get_video_poster_url( $first );
                if ( $poster ) {
                    return $poster;
                }
            }
    ...
    function tb4_home_v248_get_reel_items( $limit = 6 ) {
        ...
        if ( $query->have_posts() ) {
            while ( $query->have_posts() ) {
                ...
                $author_name = get_the_author_meta( 'display_name', $author_id ) ?: __( 'ผู้ใช้ Thinkb4do', 'thinkb4do' );
                ...
                $items[] = [
                    'title'       => get_the_title(),
                    'url'         => tb4cf_get_post_link( get_the_ID() ),
                    'image'       => $cover,
...
if ( ! function_exists( 'tb4_home_v248_fallback_reels' ) ) {
    function tb4_home_v248_fallback_reels() {
        return [
            ...
            [ 'title' => __( 'คลิป/ภาพจากชุมชน', 'thinkb4do' ), 'url' => home_url( '/community/' ), 'image' => '', 'description' => __( 'รองรับภาพ วิดีโอ และอัลบั้มจากโพสต์ชุมชน', 'thinkb4do' ), 'meta' => __( 'กำลังรอข้อมูลจริง', 'thinkb4do' ), 'badge' => __( 'ใหม่', 'thinkb4do' ), 'likes' => 0, 'comments' => 0, 'media_count' => 1 ],
        ];
    }
}
...
if ( ! function_exists( 'tb4_home_v248_fallback_community' ) ) {
    function tb4_home_v248_fallback_community() {
        return [
            [ 'title' => __( 'ชุมชน Thinkb4do', 'thinkb4do' ), 'url' => home_url( '/community/' ), 'image' => '', 'description' => __( 'พื้นที่รวมโพสต์ ถาม-ตอบ ไอเดีย โปรเจกต์ และกิจกรรมของชุมชน', 'thinkb4do' ), 'badge' => __( 'วันนี้', 'thinkb4do' ), 'likes' => 0, 'comments' => 0, 'date' => __( 'พร้อมใช้งาน', 'thinkb4do' ) ],
        ];
    }
}
        ...
        <div class="tb4-home-feed-post-list">
          ...
          ?>
            <article class="tb4-home-feed-post">
              <a class="tb4-home-feed-post-thumb" href="<?php echo esc_url( tb4cf_get_post_link( get_the_ID() ) ); ?>" aria-hidden="true" tabindex="-1">
              ...
              <div>
                ...
                <h3><a href="<?php echo esc_url( tb4cf_get_post_link( get_the_ID() ) ); ?>"><?php the_title(); ?></a></h3>
